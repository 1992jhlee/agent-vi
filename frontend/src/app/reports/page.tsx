...
export default async function ReportsPage({
  ...
  const baseUrl = `/reports?verdict=${verdict}&market=${market}`;

  return (
    <div>
      <h1 className="mb-6 text-3xl font-bold">분석 보고서</h1>

      {/* Filters */}
      <div className="mb-6 flex flex-wrap gap-2">
        <span className="text-sm text-gray-500 mr-2">투자 의견:</span>
        <FilterButton href="/reports" active={!verdict}>
          전체
        </FilterButton>
        <FilterButton
          href={`/reports?verdict=strong_buy&market=${market}`}
          active={verdict === "strong_buy"}
        >
          강력매수
        </FilterButton>
        <FilterButton
          href={`/reports?verdict=buy&market=${market}`}
          active={verdict === "buy"}
        >
          매수
        </FilterButton>
        <FilterButton
          href={`/reports?verdict=hold&market=${market}`}
          active={verdict === "hold"}
        >
          보유
        </FilterButton>
        <FilterButton
          href={`/reports?verdict=sell&market=${market}`}
          active={verdict === "sell"}
        >
          매도
        </FilterButton>
        <FilterButton
          href={`/reports?verdict=strong_sell&market=${market}`}
